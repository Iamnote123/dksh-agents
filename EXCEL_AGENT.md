...
After building: run `python scripts/recalc.py output.xlsx` and fix all `#REF!`, `#DIV/0!`, `#VALUE!`, `#NAME?` before delivering.
...
| J | Status | Formula: `=IF(F2>120,"🔴 High",IF(F2>90,"⚠️ Watch",IF(F2<30,"🔴 OOS Risk","✅ OK")))` |
...
9. Run `python scripts/recalc.py output.xlsx` — fix all errors.
...
- File: `[filename].xlsx`
- Sheets: [list]
- Rows: [count per sheet]
- Formula check: ✅ PASS / 🔴 [errors found and fixed]

**2. Key formulas applied**
List the main formulas used (for user to verify)

**3. What to update next period**
- Which cells/columns need new data
- Which cells are hardcoded inputs (yellow / blue)

**4. File path**
Where the file was saved

Example:
> "✅ Built `20260530_BigC_IMS_Tracker.xlsx` — 3 sheets (Summary, By_SKU, Raw_Data), 847 rows, formula check PASS. Update columns D–O each month with actual IMS. ABP inputs in yellow cells B2:B50 on Summary sheet. Saved to ~/Downloads/"
...
- Do NOT deliver with formula errors. Run recalc.py and fix before delivering.
